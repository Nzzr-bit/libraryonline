...
from sqlalchemy_connect import check_logpass, get_user_data_by_id, update_user_data
...
async def authenticate_user(credentials: UserCredentials):
    try:
        if check_logpass({'login': credentials.login, 'pass': credentials.password}, check_connection(connection)):
            return {"status": "success", "message": "User authenticated successfully"}
        else:
            return {"status": "error", "message": "Invalid credentials"}, status.HTTP_401_UNAUTHORIZED
    except Exception as e:
        print(e)
        return {"status": "error", "message": "Internal Server Error"}, status.HTTP_500_INTERNAL_SERVER_ERROR
...
async def update_user(user_id: int, data: UpdateUserData):
    try:
        ...
        print(e)
        result = {"error": "Failed to update user data"}
...
async def add_user(user: models.UserData):
    try:
        ...
        req_data.update({'fio': f"{user.lastname} {user.firstname} {user.middlename}", 'id_library': user.id_library,
                         'u_type': user.user_type, 'login': user.login, 'password': user.password}, )
