...
def check_logpass(req_data: dict, connection):
    ...
    if not login:
        # Если login не предоставлен, возвращаем False
        return False

    pas = [l for l in connection.execute(text("SELECT password FROM users WHERE login = :login"), {"login": login}).fetchall()]

    # Проверяем, есть ли пароль в результатах запроса
    return bool(pas)
def get_user_data_by_id(connection, user_id):
    result = connection.execute(
        text("SELECT * FROM users WHERE u_id = :user_id"),
        user_id=user_id
    ).fetchone()

    return dict(result) if result else None

def update_user_data(connection, user_id, updated_data):
    connection.execute(
        text("UPDATE users SET u_fio = :u_fio, password = :password WHERE u_id = :user_id"),
        user_id=user_id,
        fio=updated_data.get('u_fio'),
...
def get_user_id(connection, fio, id_library, u_type, login, password):
    data = {
    ...
    connection.execute(
        text(
            """INSERT INTO users(u_last_date, u_fio, id_library, u_type, login, password) VALUES(:u_last_data,:u_fio, :id_library, :u_type, :login, :password)"""),
        **data)
...
def add_student(req_data: dict, connection):
    data = {
        'user_id': get_user_id(connection, req_data.get('fio'), req_data.get('id_library'), req_data.get('u_type'), req_data.get('login'), req_data.get('password')),
